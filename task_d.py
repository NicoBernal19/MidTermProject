...
# Etiquetas del dataset Fashion-MNIST (10 categorías de ropa)
FASHION_LABELS = {
...
class SimpleGAN:
    """
    Implementación de una GAN (Generative Adversarial Network) para generar imágenes de moda.

    Una GAN tiene dos redes neuronales que compiten entre sí:
    - Generador: Crea imágenes falsas intentando engañar al discriminador
    - Discriminador: Intenta distinguir entre imágenes reales y falsas

    A través de esta competencia, el generador aprende a crear imágenes cada vez más realistas.
    """
    def __init__(self):
        self.generator = None
    ...
    def build_generator(self, latent_dim=100):
        """
        Construye la red generadora.

        El generador toma ruido aleatorio (vector latente) y lo transforma
        en una imagen de 28x28 píxeles (784 valores).

        Usa capas lineales con LeakyReLU y BatchNorm para estabilidad.
        Tanh al final para normalizar salida entre -1 y 1.
        """
        return nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.BatchNorm1d(512),
            nn.Linear(512, 784),  # 28x28 = 784 píxeles
            nn.Tanh()  # Salida entre -1 y 1
        )

    def build_discriminator(self):
        """
        Construye la red discriminadora.

        El discriminador toma una imagen de 28x28 (784 valores) y decide
        si es real o falsa (salida entre 0 y 1).

        Usa Dropout para prevenir overfitting y mejorar generalización.
        Sigmoid al final para obtener probabilidad.
        """
        return nn.Sequential(
            nn.Linear(784, 512),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.3),  # Dropout ayuda a que no se sobreajuste
            nn.Linear(512, 256),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.3),
            nn.Linear(256, 1),
            nn.Sigmoid()  # Probabilidad de ser real (0 a 1)
        )

    def train_gan(self, epochs=50, batch_size=64, latent_dim=100):
        """
        Entrena la GAN usando el dataset Fashion-MNIST.

        El proceso de entrenamiento alterna entre:
        1. Entrenar discriminador: aprende a distinguir real vs fake
        2. Entrenar generador: aprende a engañar al discriminador

        Este juego adversarial hace que ambas redes mejoren continuamente.
        """
        ...
        try:
            # Cargar dataset Fashion-MNIST (60,000 imágenes de ropa)
            ...
            # Optimizadores Adam con parámetros específicos para GANs
            ...
            for epoch in range(epochs):
                ...
                for i, (real_imgs, _) in enumerate(train_loader):
                    ...
                    # Clasificar imágenes reales (target: 1)
                    real_labels = torch.ones(batch_size, 1).to(self.device)
                    real_output = self.discriminator(real_imgs)
                    d_loss_real = criterion(real_output, real_labels)

                    # Generar imágenes falsas y clasificarlas (target: 0)
                    z = torch.randn(batch_size, latent_dim).to(self.device)
                    fake_imgs = self.generator(z)
                    fake_labels = torch.zeros(batch_size, 1).to(self.device)
                    fake_output = self.discriminator(fake_imgs.detach())  # detach para no entrenar generador
                    ...
                    # Intentar engañar al discriminador (queremos que piense que las falsas son reales)
                    fake_output = self.discriminator(fake_imgs)
                    g_loss = criterion(fake_output, real_labels)  # Truco: usar real_labels
                    g_loss.backward()
                    g_optimizer.step()
                ...
                # Guardar promedio de pérdidas por época
    ...
    def _simulate_training(self, epochs):
        ...
        Genera pérdidas sintéticas que decrecen con el tiempo.
        """
        print("Simulating GAN training...")
        for epoch in range(epochs):
            # Simular pérdidas que disminuyen gradualmente
    ...
    def generate_images(self, num_images=9, latent_dim=100):
        """
        Genera nuevas imágenes usando el generador entrenado.

        Toma vectores aleatorios (ruido latente) y los pasa por el generador
        para crear imágenes sintéticas de ropa.
        """
        if not self.is_trained:
            return None, None

        if TORCH_AVAILABLE and self.generator is not None:
            # Generar con modelo real entrenado
            self.generator.eval()
            with torch.no_grad():
                ...
                images = (images * 0.5 + 0.5) * 255  # Desnormalizar
    ...
    def _generate_simulated_images(self, num_images=9):
        """
        Genera imágenes simuladas cuando no hay modelo entrenado.
        Crea patrones geométricos simples que se parecen a ropa.
        """
        images = []
        labels = []

        for i in range(num_images):
            # Crear array base con ruido
            img_array = np.random.randint(0, 50, (28, 28), dtype=np.uint8)

            # Añadir patrones según la categoría
            category = random.randint(0, 9)
            labels.append(FASHION_LABELS[category])

            if category in [0, 2, 3, 4, 6]:  # Prendas de vestir
                # Patrón circular para simular ropa
            ...
            elif category in [5, 7, 9]:  # Calzado
                # Patrón rectangular para simular zapatos
                for x in range(10, 18):
                    for y in range(15, 25):
                        img_array[y, x] = min(255, img_array[y, x] + random.randint(100, 200))

            else:  # Bolsos
                # Patrón cuadrado para simular bolsas
                for x in range(8, 20):
    ...
    def get_training_plot(self):
        """
        Genera un gráfico mostrando la evolución de las pérdidas durante el entrenamiento.
        Idealmente, las pérdidas de ambas redes deben estabilizarse en valores similares.
        """
...
class GANVisualizer:
    """
    Maneja la interfaz gráfica para entrenar GANs y generar imágenes de moda.
    Permite entrenar el modelo, generar imágenes individuales y crear posters promocionales.
    """
    def __init__(self):
        self.gan = SimpleGAN()
    ...
    def train_model(self):
        """Entrena la GAN con el número de épocas configurado"""
        self.gan.train_gan(epochs=self.epochs_slider.value)
        self.training_plot = self.gan.get_training_plot()

    def generate_images(self):
        """
        Genera nuevas imágenes de moda usando el generador.
        Si el modelo no está entrenado, lo entrena automáticamente.
        """
    ...
    def create_poster(self):
        """
        Crea un poster promocional usando las imágenes generadas.
        Combina múltiples imágenes en un diseño atractivo con título y etiquetas.
        """
        ...
        # Crear imagen PIL para el poster
        ...
        # Añadir imágenes generadas al poster
        ...
        for i in range(num_images):
            if i < len(self.generated_images):
                ...
                if isinstance(img_array, np.ndarray):
                    ...
                    # Posicionar imágenes en grid 2x2
    ...
    def draw_interface(self, screen):
        """Dibuja toda la interfaz incluyendo controles, gráficos e imágenes"""
        # Título
    ...
    def _draw_image_grid(self, screen, start_x, start_y):
        """
        Dibuja un grid de las imágenes generadas.
        Muestra hasta 9 imágenes en una cuadrícula 3x3.
        """
        if self.generated_images is None:
            return
        ...
        for i, (img_array, label) in enumerate(zip(self.generated_images, self.image_labels)):
            ...
            if isinstance(img_array, np.ndarray):
                ...
                img_surface_array = pygame.surfarray.make_surface(
                    np.stack([img_scaled] * 3, axis=2)  # Convertir a RGB
                )
    ...
    def handle_event(self, event):
        ...
        Procesa clicks en botones y ajustes de parámetros.
        """
...
def main():
    ...
    Maneja la actualización y renderizado de la interfaz de generación de imágenes GAN.
    """
